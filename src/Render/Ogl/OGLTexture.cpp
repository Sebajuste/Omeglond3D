...
namespace OMGL3D
{
    namespace OGL
    {
        ...
        OGLTexture::OGLTexture(const std::string &name, unsigned int bpp, unsigned int width, unsigned int height) : CORE::CTexture(name, bpp, width, height), _id_name(0), _type(CORE::OMGL_COLOR_BUFFER_RGB)
        {
        ...
        void OGLTexture::Enable(unsigned int position, const CORE::TextureEnvironement &texEnv) const
        {
        ...
        void OGLTexture::Disable(unsigned int position) const
        {
            if( OGLExtention::GetInstance().glActiveTextureARB != 0 )
            {
                if( position < 8)
        ...
        void OGLTexture::SetEmptyImage(const CORE::ColorBufferType & type, bool mipmap)
        {
            ...
            if( OGLExtention::GetInstance().FrameBufferObject == true && mipmap == true)
            {
                OGLExtention::GetInstance().glGenerateMipmap(GL_TEXTURE_2D);
            }
        }

        void OGLTexture::SetImage(const CORE::CPicture & image, bool mipmap)
        {
            glBindTexture(GL_TEXTURE_2D, _id_name);
            //glEnable(GL_TEXTURE_2D);
        ...
        void OGLTexture::SetImage(const CORE::CPicture * images, std::size_t size)
        {
            if( size == 0 ) {
        ...
        void OGLTexture::GenerateTexCoord(unsigned int position, const CORE::TexCoordPlane *planes, std::size_t size)
        {
        ...
        GLuint OGLTexture::GetId() const
        {
            return _id_name;
        }

        CORE::ColorBufferType OGLTexture::GetType() const
        {
            return _type;
        }
    }
